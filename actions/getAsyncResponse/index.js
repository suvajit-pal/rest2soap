...
async function main(params) {
  try {
    ...
    const response = await ow.activations.get({name: params.activationId});

    // now verify if the requetsing actionName matches the activation id.
    if (response && response.name != action)
    throw new Error('Incorrect activation id');

    return {
      statusCode: response.response.result.statusCode || 500,
      body: response.response.result.body || response.response.result || response.response
    };
